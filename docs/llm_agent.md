...
Each role prompt is a YAML file named `<role>.yaml` with `role`, `system`, `developer_guidance`, and `output_contract` fields. This keeps scientific instructions editable without changing orchestration code.
...
Every candidate gets a structured JSON ledger. Literature grounding is mandatory and happens before hypothesis generation. The deterministic three-item checklist is kept as `deterministic_checks`; hypothesis-specific falsification tests are generated by the LLM for each hypothesis.

The `tool_planner` can request allowlisted tools only:
...
InterProScan is skipped when existing PFAM/InterPro annotations are sufficient under the default `on_demand` policy.
Tool failures are recorded as `status: error` observations so the evidence auditor and synthesizer can treat missing evidence explicitly instead of aborting the whole candidate.
...
- `ledgers/*.json`: per-candidate memory ledger containing literature, hypotheses, hypothesis-specific falsification tests, tool observations, audit findings, revisions, and final synthesis.
